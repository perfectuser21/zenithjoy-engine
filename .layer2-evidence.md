...
### C19: 无头工作流完整闭环
```bash
$ ls -la n8n/templates/headless-prompt.md n8n/scripts/*.sh n8n/workflows/cecilia-dev-loop.json
-rw-rw-r-- 1 xx xx 1879 Jan 22 ... n8n/templates/headless-prompt.md
-rwxrwxr-x 1 xx xx 1424 Jan 22 ... n8n/scripts/generate-prompt.sh
-rwxrwxr-x 1 xx xx  821 Jan 22 ... n8n/scripts/notify-n8n.sh
-rw-rw-r-- 1 xx xx 8234 Jan 22 ... n8n/workflows/cecilia-dev-loop.json

$ grep "你必须使用 /dev skill" n8n/templates/headless-prompt.md
**你必须使用 /dev skill 来执行此任务。** 这是强制性的。

$ grep "previous_result" n8n/workflows/cecilia-dev-loop.json | head -2
            {"id": "previous_result", "name": "previous_result", ...
            {"id": "previous_result", "name": "previous_result", ...

$ bash -n n8n/scripts/generate-prompt.sh && echo "语法检查通过"
语法检查通过
