...
### C20: SessionStart hook 验证
```bash
$ ls -la hooks/session-start.sh ~/.claude/hooks/session-start.sh
-rwxrwxr-x 1 xx xx 1327 Jan 22 ... hooks/session-start.sh
-rwxrwxr-x 1 xx xx 1327 Jan 22 ... /home/xx/.claude/hooks/session-start.sh

$ bash hooks/session-start.sh | grep "SKILL_REQUIRED"
[SKILL_REQUIRED: dev]
...
$ bash -n hooks/session-start.sh && echo "语法检查通过"
语法检查通过
